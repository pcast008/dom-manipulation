...
function sortData(direction) {
    ...
    const sortedArr = direction === "asc" ?
        Array.from(allItems).sort((a, b) => a.id - b.id) :
        Array.from(allItems).sort((a, b) => b.id - a.id);
    container.innerHTML = "";
    sortedArr.forEach(item => container.appendChild(item));
}
